...
@Pipe({
  name: 'distanceMinMaxRange'
})
export class DistanceMinMaxRangePipe implements PipeTransform {
  ...
  transform(items: any[], minRange?: any, maxRange?: any, condition ? :any): any[] {
    if(condition){
      if(!items){
        return [];
      }
      if(!minRange && !maxRange){
        ...
        return items.filter(item => {
          ...
          return (this.dist <= maxRange && this.dist>= minRange);
        });
      }
    }
    else{
